...
and opens the possibility to run {\small CUBEP$^3$M} on hybrid CP2010arXiv1012.0444YU-GPU clusters.
...
particle phase space array, the linked list and the buffer exchange arrays, in which case it asymptotes to about $44$ bytes per particles. 
...
In addition, this allows a high rate of particle exchange across nodes;  the memory per particle can be brought down to $32$ bytes 
if little particle passing is expected. 
However, minimizing the memory does not optimize the speed, as many {\small CPU}-intensive operations are performed on each tile -- {\small FFTW} for instance.
...
and the memory per particle is closer to 100 - 140 bytes. Also, enabling the particle ID adds about 24 bytes per particle: 8 for the main array, and 8 for the two {\small MPI}-passing buffers as well.
...
This paper describes {\small CUBEP$^3$M}, a public and  massively parallel P$^3$M N-body code that inherits from {\small PMFAST} 
 ...
 This code is fast and has a memory footprint more than two times smaller than Lean-{\small GADGET-2}. 
