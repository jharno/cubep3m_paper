...
the freedom we have is to choose the number of such layers.
 ...
 plot in a CITA128 realization, analogous to Fig. \ref{fig:den_force_fracErr}, but the pp force has been extended to  six layers of fine cells
 in the force test part of the code {\bf (do pp\_ext2 as well?)}. 
 ...
 which are now at least 5 time smaller than in the default P$^{3}$M algorithm {\bf (Check these numbers again)}.
 When averaging over 10 time steps, the improvement is only mild, as the calculation is already very accurate. 
 
 \begin{figure*}%[ht]
  \begin{center}
    \includegraphics[width=3.2in]{graphs/densityForce_ppext=6_rebin.eps}
    \includegraphics[width=3.2in]{graphs/densityForce_fracErr_ppext=6_rebin.eps}
     \includegraphics[width=3.2in]{graphs/densityForce_ppext=6_rebin_N10.eps}
    \includegraphics[width=3.2in]{graphs/densityForce_fracErr_ppext=6_rebin_N10.eps}
  ...
  except that the exact pp force has been extended to six fine mesh layers around each particle
