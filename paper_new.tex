...
%Since there is no explicit load balancing algorithm, the code runs optimally 
%for homogeneous systems such as those which prevail in large scale structures studies. 
%It has an interface with a MHD-TVD module and a radiative transfer code, 
%which opens the door to a vast number of science applications.
%or a large number of realizations for statistical studies.
%one of the fastest highest performance public N-body code. 
%Gravity is solved  on a two-level mesh with sub-grid precision due to the particle-particle (pp)
%interactions inside and across neighbouring cells. 
