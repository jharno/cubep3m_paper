...
We then test the dynamical range of the simulation, the non-Gaussian initial conditions generator and the run time halo finder algorithm 
with a third simulation configuration, which we denote the  RANGER4000, that also evolves $4000^{3}$ particles 
from $z=100$ to $z=0$ but with a box side of $3.2 h^{-1}\mbox{Gpc}$.
 Much larger runs were produced at various high performance computing centers
 around the world; their description and a comparison of their performance are also described in this paper. 
